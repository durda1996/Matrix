...
extension Matrix where Element == Float {
    ...
    static func == (lhs: Matrix<Element>, rhs: Matrix<Int>) -> Bool {
        let rhsFloat = rhs.map({ Element($0) })
        return lhs == rhsFloat
    }

    static func == (lhs: Matrix<Int>, rhs: Matrix<Element>) -> Bool {
...
extension Matrix where Element == Double {
    ...
    static func == (lhs: Matrix<Element>, rhs: Matrix<Float>) -> Bool {
        let rhsFloat = rhs.map({ Element($0) })
        return lhs == rhsFloat
    }

    static func == (lhs: Matrix<Float>, rhs: Matrix<Element>) -> Bool {
        let lhsFloat = lhs.map({ Element($0) })
        return lhsFloat == rhs
    }
